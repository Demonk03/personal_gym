...
Private mobile-first training PWA with a Flask/Supabase backend. The application opens the selected published workout directly, supports multiple programs, editable weekdays, one-day replacement or skip, workout logging, completion, history/editing, progress, body measurements, weekly review and settings. The retired pre-workout and next-day check-ins remain documented and historical records remain readable, but new answers are not collected.
...
3. For a fresh installation, apply `supabase/activate_pilot_program.sql` to approve the 10 pilot exercise cards and activate the Monday/Wednesday program. For an existing three-session pilot, apply `supabase/remove_scheduled_cardio.sql` instead: it keeps manually edited exercise statuses and historical workouts while removing the separate walking/cardio session. Then apply `supabase/multiple_programs.sql` twice. Do not re-run the production seed or pilot activation over a manually reviewed catalog.
