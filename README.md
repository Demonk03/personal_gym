...
3. Apply `supabase/activate_pilot_program.sql` to approve the 10 pilot exercise cards and activate the Monday/Wednesday program. This file is repeatable, but it must be applied again after any later run of the production seed. It keeps the previous three-session program for historical workout references; it does not schedule a separate walking/cardio session. The pilot contains real user data while its deterministic check-in thresholds still require separate clinical approval, so the PWA labels it `ПИЛОТ`.
